...
export const createPrompt = (name, description) => 
  API.post("/prompt", null, { params: { name, description, project_id: localStorage.getItem("activeProjectId") } });
...
export const createDataset = (payload) =>
  API.post("/dataset", payload, { params: { project_id: localStorage.getItem("activeProjectId") } });
...
export const runExperiment = (prompt_version_ids, dataset_id) =>
  API.post("/experiment/run", prompt_version_ids, {
    params: { dataset_id, project_id: localStorage.getItem("activeProjectId") }
  });
